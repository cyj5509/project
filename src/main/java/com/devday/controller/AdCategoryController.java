...
public class AdCategoryController {
	...
	// 일반 주소: /admin/category/secondCategory?cg_prt_code=1
	// RestFUll 개발론 주소는 /admin/category/secondCategory/1.json
	// 주소의 일부분을 값으로 사용하고자 할 경우 중괄호({})를 사용한다. 
	@ResponseBody
	@GetMapping("/secondCategory/{cg_prt_code}") // secondCategory(1차 카테고리 선택)
	public ResponseEntity<List<CategoryVO>> secondCategory(@PathVariable("cg_prt_code") Integer cg_prt_code) throws Exception {  // DB 작업을 위해 ~ throws Exception
		
		log.info("1차 카테고리 코드: " + cg_prt_code);
		
		ResponseEntity<List<CategoryVO>> entity = null;
		entity = new ResponseEntity<List<CategoryVO>>(adCategoryService.getSecondCategoryList(cg_prt_code), HttpStatus.OK);
	
		// List<CategoryVO> list = adCategoryService.getSecondCategoryList(cg_prt_code) 
